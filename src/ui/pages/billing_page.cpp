...
QString planToString(int plan)
{
    ...
    return QStringLiteral(u8"未知套餐");
}
}

BillingPage::BillingPage(QWidget* parent)
    : BasePage(QStringLiteral(u8"账单结算"),
               QStringLiteral(u8"选择统计月份后生成账单，可直接导出到指定目录。"),
               parent)
{
    setupToolbar();
    setupTable();
...
void BillingPage::setupToolbar()
{
    ...
    m_outputEdit->setPlaceholderText(QStringLiteral(u8"账单导出目录"));

    m_browseButton = new ElaPushButton(QStringLiteral(u8"浏览…"), this);
    m_computeButton = new ElaPushButton(QStringLiteral(u8"生成账单"), this);
    m_exportButton = new ElaPushButton(QStringLiteral(u8"导出账单"), this);

    layout->addWidget(new ElaText(QStringLiteral(u8"统计时间"), this));
    layout->addWidget(m_yearSpin);
    layout->addWidget(new ElaText(QStringLiteral(u8"年"), this));
    layout->addWidget(m_monthSpin);
    layout->addWidget(new ElaText(QStringLiteral(u8"月"), this));
...
void BillingPage::setupTable()
{
    ...
    m_model->setHeaderData(3, Qt::Horizontal, QStringLiteral(u8"累计时长 (分钟)"));
    m_model->setHeaderData(4, Qt::Horizontal, QStringLiteral(u8"应收金额 (元)"));
...
{
    const QLocale locale(QLocale::Chinese, QLocale::China);
    m_summaryText = QStringLiteral(u8"共计 %1 位用户，上网总时长 %2 分钟，预计应收 %3 元。")
                        .arg(userCount)
                        .arg(totalMinutes)
...
void BillingPage::updateSummaryLabel()
{
    ...
    {
        m_summaryLabel->setText(QStringLiteral(u8"还未生成账单。"));
    }
    else
    {
