...
ReportsPage::ReportsPage(QWidget* parent)
    : BasePage(QStringLiteral(u8"分析报表"),
               QStringLiteral(u8"对历史账单和使用情况做进一步分析，辅助发现异常与趋势。"),
               parent)
{
    setupContent();
}
...
void ReportsPage::setupContent()
{
    ...
    m_model->setHeaderData(0, Qt::Horizontal, QStringLiteral(u8"区间说明"));
    m_model->setHeaderData(1, Qt::Horizontal, QStringLiteral(u8"用户数"));
    m_model->setHeaderData(2, Qt::Horizontal, QStringLiteral(u8"备注"));
...
void ReportsPage::setMonthlySummary(int year, int month, const QVector<int>& usageBuckets, double totalAmount)
{
    static const QStringList bucketLabels{
        QStringLiteral(u8"0~30 小时"),
        QStringLiteral(u8"30~60 小时"),
        QStringLiteral(u8"60~150 小时"),
        QStringLiteral(u8"150 小时以上")};
    ...
    const QString text = QStringLiteral(u8"%1 年 %2 月累计应收：%3 元")
                             .arg(year)
